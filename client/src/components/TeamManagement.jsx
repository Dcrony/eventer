...
const inputCls = "w-full h-10 px-3 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium text-slate-800 outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-100 transition-all";
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-extrabold text-slate-900 tracking-tight">Team Management</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {/* Invite form */}
          {showInviteForm && (
            <form onSubmit={handleInvite} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4">
              <h3 className="text-sm font-extrabold text-slate-900 tracking-tight">Send invitation</h3>

              <div className="space-y-1.5">
                <label className="block text-xs font-bold uppercase tracking-widest text-slate-400">Email address</label>
                <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="colleague@example.com" required className={inputCls} />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                {currentRoleDesc && (
                  <p className="text-xs text-slate-500">{currentRoleDesc}</p>
                )}
              </div>

              <div className="space-y-1.5">
                <label className="block text-xs font-bold uppercase tracking-widest text-slate-400">Message <span className="normal-case font-normal">(optional)</span></label>
                <textarea
                  value={inviteMessage}
                  onChange={e => setInviteMessage(e.target.value)}
                  placeholder="Add a personal message..."
                  rows={3}
                  className="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-800 outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-100 resize-none transition-all"
                />
              </div>

              <div className="flex gap-2.5 pt-1">
                <button type="button" onClick={() => setShowInviteForm(false)}
                  className="flex-1 h-10 rounded-full border border-slate-200 bg-white text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
                  Cancel
                </button>
                <button type="submit" disabled={loading}
                  className="flex-1 h-10 rounded-full bg-pink-500 text-white text-sm font-bold hover:bg-pink-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
          ...
          <div>
            <h3 className="text-sm font-extrabold text-slate-900 mb-3 tracking-tight">
              Team members <span className="text-slate-400 font-semibold">({team.length})</span>
            </h3>
            ...
            {!loading && team.length === 0 && (
              <div className="flex flex-col items-center gap-3 py-10 text-center">
                ...
                <p className="text-xs text-slate-400">Invite people to help manage your event</p>
              </div>
            )}
            ...
            <div className="space-y-2.5">
              ...
              .map((member) => {
                ...
                return (
                  <div key={member.id || member._id} className="flex items-center gap-3 p-3.5 bg-white border border-slate-200 rounded-xl hover:border-slate-300 transition-colors">
                    {/* Avatar */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`inline-flex items-center gap-1 mt-0.5 px-2 py-0.5 rounded-full text-xs font-semibold ${ROLE_COLORS[member.role] || "bg-slate-100 text-slate-600"}`}>
                        <RoleIcon size={11} />
                    ...
                    <select
                      ...
                      className="h-8 px-2 rounded-lg border border-slate-200 bg-slate-50 text-xs font-semibold text-slate-700 outline-none focus:border-pink-400 cursor-pointer"
                    >
                      {ROLES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                    </select>
