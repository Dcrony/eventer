...
import { Building2, Globe2, MonitorPlay, X, Lock, Users, Ticket, Calendar, MapPin, Clock, Sparkles } from "lucide-react";
...
const eventTypes = [
  { id: 1, icon: <Building2 className="w-5 h-5" />, name: "In-person", hint: "Attendees will join the event at a physical location." },
  { id: 2, icon: <MonitorPlay className="w-5 h-5" />, name: "Virtual", hint: "Attendees will join the event online via a streaming platform." },
  { id: 3, icon: <Globe2 className="w-5 h-5" />, name: "Hybrid", hint: "Attendees can choose to join either in-person or online." },
];

const teamRoles = [
  { value: "co_organizer", label: "Co Organizer", description: "Full access to manage this event and the team." },
...
export default function EditEvent({ isOpen, onClose, eventId, onEventUpdated }) {
  ...
  const handleFreeToggle = (checked) => {
    ...
    setForm((prev) => {
      ...
      return { ...prev, pricing: clonePricing(previousPaidPricingRef.current) };
    });
  };
    ...
    try {
      ...
      if (teamMembers.length && updatedEvent?._id) {
        ...
        if (failedInvites.length) {
          toast.error(`Event updated, but ${failedInvites.length} team invitation(s) failed.`);
        }
      }
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-[10003] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
    ...
    >
      <div className="relative grid grid-cols-1 md:grid-cols-[2.5fr_1.1fr] bg-white rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden border border-gray-200 shadow-2xl animate-scale-in">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 my-3">
              <div className="flex flex-wrap items-start justify-between gap-4 mb-3">
                ...
                <label className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-gray-200 bg-white text-xs font-bold text-gray-600 cursor-pointer transition-all duration-200 hover:border-pink-300 hover:bg-pink-50">
                  <input type="checkbox" checked={isFree} onChange={(e) => handleFreeToggle(e.target.checked)} className="accent-pink-500" />
                  Free Event
                </label>
              </div>
              ...
              {!isFree ? (
                <div className="space-y-2">
                  {form.pricing.map((item, index) => (
                    <div key={item.type || index} className="flex items-center justify-between gap-3 p-3 rounded-xl border border-gray-200 bg-white transition-all duration-200 hover:border-pink-300">
                      <div>
              ...
              {form.visibility === "private" && (
                <div className="mt-3">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 bg-gray-50 text-gray-900 text-sm outline-none cursor-pointer font-mono"
                  />
        ...
        <div className="hidden md:flex flex-col items-center justify-center gap-3 bg-gray-50 border-l border-gray-200 px-6 py-8 text-center">
          <img src={icon} alt="Edit event" className="w-16 opacity-80 mx-auto" />
          <h2 className="text-base font-extrabold tracking-tight text-gray-900">Update your event</h2>
          <p className="text-xs text-gray-400 leading-relaxed max-w-[180px]">
            Keep your event details current, switch between paid and free ticketing, and save changes without affecting the paid checkout flow.
          </p>
        </div>
      </div>
