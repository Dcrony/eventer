...
export default function EventDetail() {
  ...
  const {
    hasAccess: canAccessLiveStreaming,
    promptUpgrade: promptUpgradeLive,
  ...
  // Tracks whether the logged-in user already holds an active free ticket
  // for this event. Set after the event loads; updated after a reservation.
    ...
    const fetchEvent = async () => {
      try {
        ...
        setSelectedTicketType(
          data.pricing?.[0] ||
            (data.isFreeEvent || data.isFree
              ? { type: "Free", price: 0 }
              : null)
        );

        // Check if the logged-in user already has a free ticket for this event
  ...
  const ticketStartingPrice = useMemo(() => {
    ...
    const lowest = Math.min(
      ...event.pricing.map((t) => Number(t.price || 0))
    );
  ...
  const reserveFreeTicket = async () => {
    try {
      await API.post("/tickets/create", {
        eventId: event._id,
        quantity: 1, // always 1 for free tickets
      ...
      setEvent((current) =>
        current
          ? {
              ...current,
              ticketsSold: Number(current.ticketsSold || 0) + 1,
              totalTickets: Math.max(0, Number(current.totalTickets || 0) - 1),
            }
          : current
      );
      ...
      toast.error(
        error.response?.data?.message || "Failed to reserve ticket"
      );
    }
  };
  ...
  const handleLike = async () => {
    ...
    setEvent((current) => ({
      ...
      likeCount: Math.max(
        0,
        Number(current.likeCount || 0) + (nextLiked ? 1 : -1)
      ),
    }));
  ...
  const freeTicketClaimed = isEventFree && alreadyHasFreeTicket;
  const buyDisabled = isSoldOut || freeTicketClaimed;
  const buyLabel = isSoldOut
    ? "Sold out"
    : freeTicketClaimed
    ? "Already reserved"
    : "Get tickets";
                  ...
                  >
                    ...
                    <div className="space-y-0.5">
                      ...
                      <small className="block text-xs text-gray-400">
                        @{event.createdBy?.username || "tickispot"}
                      </small>
                    </div>
                  </div>
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-3 gap-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                      {[
                        ...
                        },
                        {
                          Icon: MapPin,
                          label: "Location",
                          value: event.location || "Online event",
                        },
                        {
                          Icon: Users,
                          label: "Community",
                          value: `${event.ticketsSold || 0} attendees already in`,
                        },
                        ...
                        >
                          ...
                          />
                          <div>
                            <strong className="block text-xs font-bold text-gray-900 mb-0.5">
                              {label}
                            </strong>
                  ...
                  <section>
                    ...
                    <div className="mt-2">
                      {teamLoading ? (
                        ...
                        <p className="text-gray-400 text-sm">
                          No team members have joined this event yet.
                        </p>
                      )}
                    </div>
                  </section>
                            ...
                            <div className="absolute inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-10">
                              <div className="text-center text-white p-6 max-w-[280px]">
                                ...
                                <p className="text-sm mb-4 opacity-90">
                                  Upgrade to Pro to watch live streams from
                                  events.
                                </p>
                                <Button
              ...
              <aside className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 lg:p-6 h-fit">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {event.pricing?.length ? (
                      event.pricing.map((ticketType) => (
                        ...
                        >
                          <div className="space-y-0.5">
                            ...
                            <span className="text-xs text-gray-400 block">
                              {ticketType.benefits ||
                                "Access to the event experience"}
                            </span>
                          </div>
                      ...
                      <div className="flex justify-between items-center gap-3 p-3 rounded-lg border-2 border-gray-200 bg-white">
                        ...
                        <strong className="text-sm font-extrabold text-pink-500">
                          Free
                        </strong>
                      </div>
                    )}
                  ...
                  {/* Quantity selector
                      Free events: locked to 1 (server also enforces this).
                      Paid events: 1–5 as before. */}
                  <div className="space-y-1">
                    <label
                      htmlFor="event-quantity"
                      className="text-[0.65rem] font-bold uppercase tracking-wider text-gray-400"
                    >
                      Quantity
                    </label>
                    <select
                    ...
                    {isEventFree && (
                      <p className="text-[0.65rem] text-gray-400">
                        Free events are limited to 1 ticket per person.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="text-center p-3 rounded-lg bg-gray-50 border border-gray-200">
                      <strong className="block text-lg font-extrabold tracking-tight text-gray-900">
                        {remainingTickets}
                      </strong>
